...
def readexactly(sock, numbytes):
    ...
    while len(data) < numbytes:
        remaining = numbytes - len(data)
        chunk = sock.recv(remaining)
        if not chunk:
            logging.error("Connection closed before reading complete data")
...
def kill_game(game):
    ...
    logging.debug("Terminating game between players")
...
def compare_cards(card1, card2):
    ...
    rank1 = card1 % 13
    rank2 = card2 % 13
    result = -1 if rank1 < rank2 else (1 if rank1 > rank2 else 0)
    logging.debug("Comparing cards: %d (rank=%d) vs %d (rank=%d) -> %d", 
                 card1, rank1, card2, rank2, result)
    return result
...
def serve_game(host, port):
    """War game server that runs forever serving client pairs"""
    async def handle_game(p1, p2):
        """Handle one complete game between two players"""
        p1_reader, p1_writer = p1
        p2_reader, p2_writer = p2
        
        try:
            # Verify both players
            for i, (reader, writer) in enumerate([p1, p2], 1):
                cmd = await reader.readexactly(2)
                if cmd != bytes([Command.WANTGAME.value, 0]):
                    raise ValueError(f"Player {i}: Bad WANTGAME")

            # Deal and send cards
            hand1, hand2 = deal_cards()
            p1_writer.write(bytes([Command.GAMESTART.value]) + bytes(hand1))
            p2_writer.write(bytes([Command.GAMESTART.value]) + bytes(hand2))
            await asyncio.gather(p1_writer.drain(), p2_writer.drain())
            ...
            for _ in range(26):
                ...
                if p1_card[0] != Command.PLAYCARD.value or p2_card[0] != Command.PLAYCARD.value:
                    raise ValueError("Invalid PLAYCARD")

                # Compare cards
                result = compare_cards(p1_card[1], p2_card[1])
                res1 = Result.WIN if result == 1 else Result.LOSE if result == -1 else Result.DRAW
                res2 = Result.LOSE if result == 1 else Result.WIN if result == -1 else Result.DRAW

                # Send results
                p1_writer.write(bytes([Command.PLAYRESULT.value, res1.value]))
                p2_writer.write(bytes([Command.PLAYRESULT.value, res2.value]))
                await asyncio.gather(p1_writer.drain(), p2_writer.drain())
            ...
            await asyncio.gather(
                p1_writer.wait_closed(),
                p2_writer.wait_closed()
            )
    ...
    async def run_server():
        ...
        async def handle_connection(reader, writer):
            ...
            if len(waiting) >= 2:
                # Start new game with first two waiting clients
                asyncio.create_task(handle_game(waiting.pop(0), waiting.pop(0)))
...
async def client(host, port, loop):
    ...
    try:
        reader, writer = await asyncio.open_connection(host, port)
        logging.debug("[WANT GAME 0]")
        writer.write(b"\0\0")
        card_msg = await reader.readexactly(27)
        logging.debug("[GAME START 1]")

        myscore = 0
        for card in card_msg[1:]:
            logging.debug("[PLAY CARD 2]: card=%d", card)
            writer.write(bytes([Command.PLAYCARD.value, card]))
            result = await reader.readexactly(2)
            if result[1] == Result.WIN.value:
                myscore += 1
                logging.debug("[PLAY RESULT 3] (win): score = %d", myscore)
            elif result[1] == Result.LOSE.value:
                logging.debug("[PLAY RESULT 3] (lose): score = %d", myscore)
                myscore -= 1
        if myscore > 0:
            result = "won"
        elif myscore < 0:
